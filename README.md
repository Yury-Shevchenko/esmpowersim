...
Reproducible simulator and planning tool for the methods paper *"Powering event- and
location-contingent experience sampling"*.

Every widely used ESM/EMA power tool assumes the number of measurement occasions per person is a
quantity the researcher **sets**. When prompts are **event-contingent** (the participant reports when
something happens) or **location-contingent** (a geofence fires), that assumption breaks: each
person's number of observations is a **random variable** driven by how often their triggering context
occurs — and that rate varies substantially between people.

This package implements the preregistered data-generating mechanism — a **trigger process** making
...
> **Status: research code accompanying a manuscript that has not yet been peer reviewed.** Three of
> the four preregistered hypotheses were not supported at their thresholds, and the paper reports that
> plainly; the planner's outputs reflect the same results. The default variance components in
...
| `R/grid.R` | Preregistered primary (144 cells) + secondary S1–S5 + a `smoke` grid |
| `R/run.R` | CLI driver → results CSV |
| `R/validate.R` | Pre-run checks (prereg §7): Lafit special case, recovery, DGM sanity |
| `R/analyze.R` | Confirmatory H1–H4 tests (prereg §5) + power/convergence surfaces → `analysis-summary.md` |
| `R/repro.R` | Archives sessionInfo + git SHA + args next to every results file |
| `R/lookup.R` | Instant answers from the confirmatory grid (same contract as `run_cell()`) |
...
# pre-run validation (must pass before the confirmatory run)
Rscript R/validate.R --R=300 --seed=20260709

# confirmatory run (heavy — 144 cells x 2000 reps; --cores parallelises, fork-based)
...
# confirmatory analysis: H1-H4 verdicts + figures (prereg §5)
Rscript R/analyze.R --primary=results/primary.csv \
    --secondary=results/secondary.csv --outdir=results/analysis
```

Requires R with `lme4`, `MASS`, `Matrix`, `nlme`. Pin versions with `renv` before the confirmatory
run and archive `sessionInfo()` (prereg §6). `brms`/`glmmTMB` cross-checks (prereg §3-M) are optional
...
Reads the results CSV and applies the **fixed** prereg §5 criteria:
...
- **Lookup (default, instant).** Reads the confirmatory grid (504 cells, R = 2000/1000). An exact hit
...
`Dockerfile` pins the exact environment the confirmatory run used (R 4.3.1, lme4 1.1-34, Matrix 1.6-1);
...
If it ever fails, the "same engine as the preregistered study" claim is false and the wording — not
